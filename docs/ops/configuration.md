...
| `Worker__MaxReportArchiveEntries` | `512` | Cap on archive entries examined per attachment, so an archive of millions of tiny members costs bounded work even when nothing inside it is large. Counted before the `.xml`/`.json` filter, because an attacker picks the names. The two ingestion paths answer differently on purpose. On the **mailbox** path entries past the cap are ignored and a warning is logged — mail cannot be re-delivered on request, so taking what fits beats refusing it. On **`POST /api/v1/reports`** the whole request is refused with a 413 naming this setting, because that caller can split the archive and post again, and a 200 reading `inserted: 5, failed: 0` for a twenty-entry archive is indistinguishable from having sent five. |
...
| `Worker__ReportIngestRateLimitPermits` | `60` | Requests one credential may make to `/api/v1/reports` per window. The size ceiling bounds one request; this bounds how many, so a leaked credential cannot post the maximum payload as fast as the network allows. Partitioned by credential, not address, so a pipeline behind NAT cannot starve an unrelated tenant. Exceeding it is a 429 with `Retry-After`. |
| `Worker__ReportIngestRateLimitWindowSeconds` | `60` | Window the permit count applies to. The default pair is far above any real reporting pipeline — aggregate reports arrive daily, not continuously. |
