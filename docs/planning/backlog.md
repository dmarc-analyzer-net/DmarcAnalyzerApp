...
- [ ] (todo) Give a self-hoster a way back in after losing the admin password. There is none today: no reset endpoint, no forgot-password flow, and `POST /api/v1/users` needs an existing admin session, so the only routes are hand-crafting a PBKDF2-SHA256 hash straight into `agency_user` or deleting every user to re-trigger bootstrap — which also drops `user_client_grant` rows and any viewer scoping with them (`RequiresBootstrapAsync` is `!AnyAsync()`, so nothing short of an empty table re-opens registration). Hit for real on the dev instance. An `APP_MODE=reset-password` one-shot would fit the existing `migrate` mode precedent and make it a documented operation rather than archaeology. Worth pairing with a note that the product ships as "create the first admin account and you are running", which is exactly the user who will need this.
