...
- [x] (done) Surface RFC 9990's `pass` action disposition in analytics and the console, in the same change that taught the parser to preserve it. Splitting the two would have shipped a screen that contradicts itself: the rollups bucket by exact string, so a preserved `pass` counted in the message total while accounting for none of the breakdown — an all-`pass` source reading `none · 0 / quarantine · 0 / reject · 0` under "7 of 7 messages compliant". `AnalyticsDispositionsDto` gained a `Pass` member, both projections that build it gained a filter, and the four badges moved into an exported `DispositionBadges` so the row could be asserted without standing up the whole detail page. The RFC's own semantics are why this is a fourth bucket rather than a fold into `none`: `none` is "no action taken", `pass` is "no action, passing DMARC w/enforcing policy" — a receiver that enforced and let the mail through, which is the opposite of declining to act. Storage needed nothing: `Disposition` is already `varchar(32)`. Worth knowing for the next reader: **no historic data can be backfilled.** Until the parser change, a reported `pass` was rewritten to `none` before persistence and no raw XML is retained, so the two are indistinguishable in every row already stored — the only surviving copy is the original message in the mailbox.
