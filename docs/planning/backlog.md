...
- [ ] (todo) Fix the last correlated subquery, in `ListDomainAnalyticsAsync`. `Reporters` counts distinct `OrganizationName`, which lives on the parent report, so despite the flattened projection EF still emits a per-domain subplan that re-joins `dmarc_report` twice (as both `d2` and `d3`). This makes `/analytics/domains` the slowest endpoint left in the app: 1,988ms wall, of which 1,930ms is that single query. The window scan beneath it now finishes in 239ms, then `GroupAggregate` burns ~2.0s to emit 30 rows — `Buffers: shared hit=613862 read=269460` above the subplan versus 7,656 below it. Note the method's own comment already claims this pathology was avoided, which is true for `Reports` and `Sources` (both plain record columns) but not for `Reporters`.
