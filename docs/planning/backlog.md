...
- [ ] (todo) Retire the two normalization steps DmarcRua 2.0.1 made redundant, now that the upgrade has landed. Neither is harmful, so neither blocked the upgrade, but both now do work the library does — and one of them loses data. (a) **SPF `scope=helo` is still rewritten to `mfrom`.** 2.0.1 added `SpfDomainScope.Helo`, so the scope can finally be stored as sent; today every `helo` result is recorded as `mfrom`, which is a wrong value in the database rather than a missing one, and the warning explaining it is discarded by `MailboxSyncService` (see the validation-warnings item above). Changing it alters stored data and `DmarcRuaReportParserTests`' `scope-helo-1` assertion, and is worth doing deliberately rather than as part of a dependency bump. (b) **Namespace stripping is now belt-and-braces.** 2.0.1 deserializes through a `NamespaceIgnorantXmlReader` and dropped the hardcoded `http://dmarc.org/dmarc-xml/0.1` from its `XmlType` attributes, so `NormalizeReportXml`'s namespace pass no longer changes any outcome — it only sets `HasValidationWarnings` and emits a `stripped XML namespace` warning on every DMARCbis report. Keep the pass or drop it, but the warning is now noise. Note DmarcRua declares `NamespaceIgnorantXmlReader` as a `public` type in the *global* namespace, so it is visible unqualified everywhere in the API project; do not accidentally reference it.
